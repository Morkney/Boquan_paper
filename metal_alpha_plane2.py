...
FeH_bins = np.linspace(*binrange[0], N_bins)
MgFe_bins = np.linspace(*binrange[1], N_bins)
hist = np.histogram2d(data['FeH '][cut], data['MgFe'][cut], weights=data['GAGE'][cut], bins=[FeH_bins, MgFe_bins])[0]
...
# Convert histogram into time values:
...
time_hist = np.interp(hist, a, lookback_time)

# Plot contours:
contours = [2, 4, 6, 8, 10, 12]
CS = ax.contour(time_hist.T, extent=extent, levels=contours, colors=['k'], zorder=99, linewidths=[1])
...
plt.savefig('./images/metal_plane_Au-%i_r%i-%i_time' % (halo, r_cut[0], r_cut[1]), bbox_inches='tight')
