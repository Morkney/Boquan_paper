...
ax[1].scatter(SubSnapNum[Rank], Order[Rank], s=DotSize[Rank]**2, c=c[Rank], linewidths=0.5, **scatter_kwargs)
for SubSnap in range(SubSnapNum.max()-1, SubSnapNum.min()-1, -1):
  ...
  # Draw the nodes:
  ax[1].scatter(SubSnapNum[Rank&~satellite], Order[Rank&~satellite], s=DotSize[Rank&~satellite]**2, c=c[Rank&~satellite], linestyle='-', linewidths=0.5, **scatter_kwargs)
  ax[1].scatter(SubSnapNum[Rank&satellite], Order[Rank&satellite], s=DotSize[Rank&satellite]**2, c=c[Rank&satellite], linewidths=0, **scatter_kwargs)

  # Add the connecting lines:
  for j in range(np.sum(Rank)):
    if LabelHalos:
      string = '%i' % HaloNum[Rank][j] + '\n' + '%i' % SnapNum[Rank][j]
      ax[1].text(SubSnapNum[Rank][j], Order[Rank][j], string, fontsize=1, ha='center', va='center')
    edge_c = LineColour
...
ax[1].set_xticklabels(['%.2g' % round(i, tick_precision) for i in new_tick_labels])
ax[1].set_xlabel(r'Redshift', fontsize=fs)
ax[1].tick_params(axis='x', labelsize=fs-2)
ax[1].minorticks_off()

ax[1].set_yticks([])
ax[1].tick_params(axis='x', which='both', direction='out')
ax[1].tick_params(top=False, which='both')
for spine in ['left', 'right', 'top']:
  ax[1].spines[spine].set_visible(False)

if direction == 'left to right':
  ax[1].set_xlim(new_tick_locations[[-1,0]])
elif direction == 'right to left':
  ax[1].set_xlim(new_tick_locations[[0,-1]])
#--------------------------------------------------------------------
...
nodeColour = cmap(64)
ax[1].scatter([],[], s=DotSizes[-1]**2, c=nodeColour, edgecolors='k', linewidths=0.5, label=r'Central')
ax[1].scatter([],[], s=DotSizes[-1]**2, c=nodeColour, edgecolors='k', linewidths=0., label=r'Satellite $(R_{\rm G}<R_{200})$')
ax[1].scatter([],[], c='None', label=' ')
ax[1].scatter([],[], c='None', label=' ')
for DotSize, DotMass in zip(DotSizes, DotMasses):
  ax[1].scatter([],[], s=DotSize**2, c='black', lw=0, label=r'$%s\,$M$_{\odot}$' % latex_float(10**DotMass))

# Add labels for line types:
ax[1].plot([],[], color=MainProgLineColour, lw=4, label='Main progenitor line')
...
if LabelRatios:
  ...
  for MergeSnap in np.unique(f['SnapNum'][:][Desc[MergeID]]):
    ...
    ax[1].plot([MergeSnap, MergeSnap+xpad], [MergeOrder, MergeOrder+ypad], 'k-', lw=0.5)
    ax[1].text(MergeSnap+xpad, MergeOrder+ypad, merge_string, fontsize=fs-4, ha='center', va='bottom')  
#--------------------------------------------------------------------
...
ax[1].text(0.5, height, 'Au-%i merger tree' % halo, fontsize=fs, ha='center', va='top', transform=ax[1].transAxes)
#--------------------------------------------------------------------
...
header, cat, lf, data = get_data(halo, MaxSnapNum, data_fields=data_fields, cat_fields=cat_fields)

# Binning:
time_bins = np.linspace(0.8, 13.8, 500)
a_bins = auriga.time_age(time_bins)
snap_bins = np.linspace(snapshots.min(), snapshots.max(), len(snapshots)*10)
a_bins = np.interp(snap_bins, snapshots, 1/(1+redshifts))
